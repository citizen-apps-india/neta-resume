    ...
    <>
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "28px clamp(14px,4vw,28px) 64px", width: "100%" }}>
        ...
        <p style={{ fontSize: 15, color: "var(--ink2)", margin: "0 0 20px", maxWidth: "66ch" }}>
          Each {data?.house ?? (house === "rs" ? "Rajya Sabha" : "18th Lok Sabha")} party&rsquo;s questions, broken down by policy theme — the topics its
          members collectively emphasise. A descriptive comparison of focus, from the official record; never a ranking of merit.
        </p>

        <HouseToggle house={house} hrefLs={`/parliament/parties${focus ? `?focus=${encodeURIComponent(focus)}` : ""}`} hrefRs={`/parliament/parties?house=rs${focus ? `&focus=${encodeURIComponent(focus)}` : ""}`} />
