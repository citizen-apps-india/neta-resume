    ...
    <div style={{ display: "grid", gap: 10 }}>
      {items.map((m) => (
        <Link
          ...
          style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "3px 12px", alignItems: "baseline", textDecoration: "none", color: "var(--ink)" }}
        >
          ...
          <div style={{ gridColumn: "1 / -1", height: 7, background: "var(--rule)", borderRadius: 4, overflow: "hidden" }}>
            <div style={{ width: `${(m.count / max) * 100}%`, height: "100%", background: themeColor(m.theme), borderRadius: 4 }} />
          </div>
        </Link>
      ))}
    </div>
